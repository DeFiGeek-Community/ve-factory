...
contract VeFactoryTest is Test, DeployVeFactory {
    ...
    function testCreateVeToken() public {
        ...
        assertEq(
            entries[0].topics[0], keccak256("VeTokenCreated(address,address,string,string)"), "Event signature mismatch"
        );
        assertEq(entries[0].topics[1], bytes32(uint256(uint160(tokenAddr))), "Token address topic mismatch");
        assertEq(entries[0].topics[2], bytes32(uint256(uint160(veTokenAddr))), "VeToken address topic mismatch");
        ...
        IVeFactory.VeTokenInfo memory info = factory.getDeployedVeTokens(tokenAddr);
    ...
    function testMultipleVeTokenCreation() public {
        ...
        // 3つのveTokenを作成
        address veTokenAddr1 = factory.createVeToken(tokenAddr, "veTokenName1", "veTKN1");
        address veTokenAddr2 = factory.createVeToken(address(token2), "veTokenName2", "veTKN2");
        address veTokenAddr3 = factory.createVeToken(address(token3), "veTokenName3", "veTKN3");
        ...
        // それぞれのveTokenの情報を確認
        IVeFactory.VeTokenInfo memory info1 = factory.getDeployedVeTokens(tokenAddr);
        IVeFactory.VeTokenInfo memory info2 = factory.getDeployedVeTokens(address(token2));
        IVeFactory.VeTokenInfo memory info3 = factory.getDeployedVeTokens(address(token3));
    ...
    function testGetDeployedVeTokensWithNonexistentAddress() public view {
        ...
        IVeFactory.VeTokenInfo memory info = factory.getDeployedVeTokens(address(1)); // 存在しないアドレスを指定
        assertEq(info.tokenAddr, address(0)); // 存在しない場合、tokenAddrはアドレスゼロであるべき
    }

    function testCreateVeTokenWithFuzzing(string memory _name, string memory _symbol) public {
        ...
        assertTrue(veTokenAddr != address(0), "veToken creation failed with valid inputs");
