...
contract VeFactory is UUPSUpgradeable, OwnableUpgradeable {
    ...
    event VeTokenCreated(address indexed tokenAddr, address indexed veTokenAddr, string name, string symbol);
    ...
    function createVeToken(address _tokenAddr, string memory _name, string memory _symbol) external returns (address) {
        ...
        require(
            $.deployedVeTokens[_tokenAddr].veTokenAddr == address(0), "veToken for this token address already exists."
        );
    ...
    function getDeployedVeTokens(address tokenAddr) external view returns (VeFactorySchema.VeTokenInfo memory) {
        VeFactorySchema.VeFactoryStorage storage $ = VeFactoryStorage.Factory();
